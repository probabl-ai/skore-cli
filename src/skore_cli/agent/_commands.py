"""The ``skore agent`` click group: ``init`` and ``status``."""
...
click.rich_click.COMMAND_GROUPS = {
    **getattr(click.rich_click, "COMMAND_GROUPS", {}),
    "cli agent": [
        {"name": "Workspace", "commands": ["init", "status"]},
    ],
}
...
def _resolve_hub_workspace(
    ...
    if cred.kind != "bearer":
        # No credential: nothing can be attached; init already warns about this.
        return None
    ...
    if not workspaces:
        raise click.ClickException(
            "you are not a member of any hub workspace; create or join one, then "
            "re-run `skore agent init`."
        )
...
@click.option(
    ...
    help=f"Base URL of the Skore Hub agent endpoint (default: {DEFAULT_HUB_URL}).",
)
@click.option(
    "--hub-workspace",
    default=None,
...
def status(workspace: Path) -> None:
    ...
    if not marker_path.exists():
        raise click.ClickException(
            f"no {MARKER_FILENAME} in {workspace}; run `skore agent init` first."
        )
